...
class Coach:
    ...
    def run(self):
        ...
        if (args.model).upper() == ('REGCN').upper():  
            ...
            time_stamp = {'YAGO': 1,'WIKI': 1,'ICEWS14': 1,'ICEWS18': 24,'ICEWS14cut10': 1,'ICEWS14cut20': 1,'ICEWS14cut50': 1,'ICEWS14s': 1}
        ...
        if args.mode == 'test':  
            ...
            if os.path.exists(model_state_file):
                ...
                log('all Test: MRR = %.2f, Hits1 = %.2f, Hits3 = %.2f, Hits10 = %.2f' % (MRR*100, Hits1*100, Hits3*100, Hits10*100), save=False, oneline=False)

                MRR, Hits1, Hits3, Hits10 = self.testEpoch(mode='test', model_state_file=model_state_file, filter_relation=5)
                log('5 Test: MRR = %.2f, Hits1 = %.2f, Hits3 = %.2f, Hits10 = %.2f' % (MRR*100, Hits1*100, Hits3*100, Hits10*100), save=False, oneline=False)

                MRR, Hits1, Hits3, Hits10 = self.testEpoch(mode='test', model_state_file=model_state_file, filter_relation=10)
                log('10 Test: MRR = %.2f, Hits1 = %.2f, Hits3 = %.2f, Hits10 = %.2f' % (MRR*100, Hits1*100, Hits3*100, Hits10*100), save=False, oneline=False)

                MRR, Hits1, Hits3, Hits10 = self.testEpoch(mode='test', model_state_file=model_state_file, filter_relation=20)
                log('20 Test: MRR = %.2f, Hits1 = %.2f, Hits3 = %.2f, Hits10 = %.2f' % (MRR*100, Hits1*100, Hits3*100, Hits10*100), save=False, oneline=False)

                MRR, Hits1, Hits3, Hits10 = self.testEpoch(mode='test', model_state_file=model_state_file, filter_relation=30)
                log('30 Test: MRR = %.2f, Hits1 = %.2f, Hits3 = %.2f, Hits10 = %.2f' % (MRR*100, Hits1*100, Hits3*100, Hits10*100), save=False, oneline=False)

                MRR, Hits1, Hits3, Hits10 = self.testEpoch(mode='test', model_state_file=model_state_file, filter_relation=40)
                log('40 Test: MRR = %.2f, Hits1 = %.2f, Hits3 = %.2f, Hits10 = %.2f' % (MRR*100, Hits1*100, Hits3*100, Hits10*100), save=False, oneline=False)

                MRR, Hits1, Hits3, Hits10 = self.testEpoch(mode='test', model_state_file=model_state_file, filter_relation=50)
                log('50 Test: MRR = %.2f, Hits1 = %.2f, Hits3 = %.2f, Hits10 = %.2f' % (MRR*100, Hits1*100, Hits3*100, Hits10*100), save=False, oneline=False)
            else:
        ...
        # 训练新的模型
            ...
            for ep in range(0, args.max_epochs): 
                ...
                # if early_stop==3:  # 当模型收敛的时候，更换loss函数，引入置信度，让模型及进行自信学习，
                #     # break
                #     if self.loss_check:
                #         break
                #     else:
                #         MRR, Hits1, Hits3, Hits10 = self.testEpoch(mode='test',model_state_file=model_state_file)
                #         log('First Test: MRR = %.2f, Hits1 = %.2f, Hits3 = %.2f, Hits10 = %.2f\n' % (MRR*100, Hits1*100, Hits3*100, Hits10*100), save=False, oneline=False)
                #         log("Select confident samples to guide model training...")
    ...
    def trainEpoch(self):
        ...
        for idx in idx_list:
            ...
            if (args.model).upper() == ('CYGNET').upper():
                if type(data_pred)==dict:
                    data_pred = torch.LongTensor(list(data_pred.keys())).to(self.device)
                time_tail_freq_idx = idx
                all_tail_seq = self.time_tail_freq_list[time_tail_freq_idx]
                seq_idx = data_pred[:, 0] * self.handler.num_r * 2 + data_pred[:, 1]  # T时刻的(h,r)对
                history_tail_seq = torch.tensor(all_tail_seq[seq_idx.cpu().numpy()].todense())
                one_hot_tail_seq = history_tail_seq.masked_fill(history_tail_seq != 0, 1)

            if ((args.model).upper() == ('TRANSE').upper()) or ((args.model).upper() == ('CONVE').upper()) or ((args.model).upper() == ('ROTATE').upper()): 
                if type(data_pred)==dict:
                    data_pred = torch.LongTensor(list(data_pred.keys())).to(self.device)
                num_batches = data_pred.shape[0] // self.batch_size + (1 if data_pred.shape[0] % self.batch_size != 0 else 0)
                for i in range(num_batches):
                    data_batch = data_pred[i * self.batch_size : (i + 1) * self.batch_size]
                    loss,_,_ = self.model.calculate_loss(data_batch)
                    loss.backward()
    ...
    def testEpoch(self, mode='test', model_state_file=None, filter_relation = -1):
        Ranks, Ranks_noise = [], []
        if mode=='test':
        ...
        with torch.no_grad():
            ...
            for idx in idx_list:
                ...
                if filter_relation>0:
                    data_pred = data_pred[data_pred[:, 1] <= filter_relation]  # 筛选长尾关系的样本出来评价
                    # data_pred = data_pred[data_pred[:, 1] <= self.handler_raw.num_r]  # 筛选长尾关系的样本出来评价
                    # data_pred = data_pred[data_pred[:, 1] > filter_relation]  # 筛选长尾关系的样本出来评价
                    # data_pred = data_pred[data_pred[:, 1] <= self.handler_raw.num_r]  # 筛选长尾关系的样本出来评价

                one_hot_tail_seq = None
                if (args.model).upper() == ('CYGNET').upper():
                    time_tail_freq_idx_start = len(self.handler.trnListDirt) + len(self.handler.valListDirt) if mode=='test' else len(self.handler.trnListDirt)
                    time_tail_freq_idx = idx + time_tail_freq_idx_start
                    all_tail_seq = self.time_tail_freq_list[time_tail_freq_idx]
                    seq_idx = data_pred[:, 0] * self.handler.num_r * 2 + data_pred[:, 1]  # T时刻的(h,r)对
                    history_tail_seq = torch.tensor(all_tail_seq[seq_idx.cpu().numpy()].todense())
                    one_hot_tail_seq = history_tail_seq.masked_fill(history_tail_seq != 0, 1)          

                if ((args.model).upper() == ('TRANSE').upper()) or ((args.model).upper() == ('CONVE').upper()) or ((args.model).upper() == ('ROTATE').upper()):  # TransE 模型，考虑到显存消耗，需要分batch进行输入
                    if type(data_pred)==dict:
                        data_pred = torch.LongTensor(list(data_pred.keys())).to(self.device)
                    num_batches = data_pred.shape[0] // self.batch_size + (1 if data_pred.shape[0] % self.batch_size != 0 else 0)
                    for i in range(num_batches):
                        data_batch = data_pred[i * self.batch_size : (i + 1) * self.batch_size]
                        score_pred= self.model.predict(data_batch)
                        if mode =='test':
                            # print("filter metrics!")
                            ranks = get_ranks(data_batch, score_pred, batch_size=-1, filter=args.filter, num_e=self.handler.num_e, facts_filter=self.facts_filter)
                        else:
                            ranks = get_ranks(data_batch, score_pred, batch_size=-1, filter=0, num_e=self.handler.num_e, facts_filter=self.facts_filter)
                        Ranks.extend(ranks)
                else:
                    
                    if self.use_query:
                        self.model.forward(data_history_all, kg_dict_pred, one_hot_tail_seq, confidence=0)
                        # self.model.forward_observe(data_history_all, kg_dict_pred, one_hot_tail_seq, confidence=0, dataset=args.dataset)  # 存储并观测表示的变化
                    else:
                        self.model.forward_old(data_history_all, one_hot_tail_seq, confidence=0)
...
if __name__ == '__main__':
    ...
    model_state_file = model_dir+'/{}_init{}_attack{}_noise{}+{}_lr{}_histlen{}_temp{}_bypass{}_context{}_conf{}_usequery{}_useconf{}'.format(
        args.model, args.init_method, args.attack, args.neg_ratio, args.noise_strategy, args.lr, args.history_len, args.lambda1, args.lambda2, args.lambda3, args.conf_threshold, args.use_query, args.use_conf)
