...
class DataHandler():
    ...
    #     print(num_r)
    #     trnData, trnTime = self.load_file(os.path.join(Data_file_path,self.data_name)+'/train.txt', num_r=num_r)
    #     valData, valTime = self.load_file(os.path.join(Data_file_path,self.data_name)+'/valid.txt', num_r=num_r)
    #     tstData, tstTime = self.load_file(os.path.join(Data_file_path,self.data_name)+'/test.txt', num_r=num_r)
    ...
    #         print("finish one snapshot")
    #         return 0

    #     kg_data_args = [(idx, tim) for idx, tim in enumerate(all_times)]
    #     with ThreadPoolExecutor(max_workers=20) as executor:
    #         for result in executor.map(history_freq, kg_data_args):
    #             pass
    

    def calculate_history_freq_noise(self,):
        num_e, num_r = self.num_e, self.num_r
        all_times = self.allTime
        if args.neg_ratio:
            all_data = self.trnListDirt + self.valListDirt + self.tstListDirt
            all_data_tmp = []
            for kg in all_data:
                all_data_tmp.extend(list(kg.keys()))
        else:
            all_data = self.trnList +  self.valList + self.tstList
            all_data_tmp = []
            for kg in all_data:
                all_data_tmp.extend(kg)
        all_data = np.array(all_data_tmp)
        # all_data_set = set((s,r,o,t) for s,r,o,t in all_data)
        save_dir_obj = data_dir + '/history_seq{}'.format(args.neg_ratio)
        if not os.path.exists(save_dir_obj):
            os.makedirs(save_dir_obj)
...
if __name__ == '__main__':
    ...
    handler.calculate_history_freq_noise()

    log('Finished history freq: {}'.format(args.dataset))
